...
Class Curricula_Model extends CI_Model{
	...
	*/
	public function getAllPaperSetterRecords($tbl_name, $col = ' * ', $condition=null, $order_by = NULL,$limit=NULL, $start=NULL, $likeSearch=NULL)
    {
        ...
        $this->db->select($col);
        $this->db->where('is_deleted', '0');
        $this->db->where(('moderator_status="2" OR moderator_status="3"'));
        $this->db->where('is_deleted', '0');
		if(!empty($condition))
		{ 
			foreach($condition as $key=>$val) {
	...
	*/
	public function getAllQBankRecords($tbl_name, $col = ' * ', $condition=null, $where_in=NULL, $where_key = 'id', $find_in_set = '')
    {
		...
		if($find_in_set) {
			//$this->db->where("FIND_IN_SET(paper_setter,'".$find_in_set."') OR FIND_IN_SET(moderator_id,'".$find_in_set."')",null,false);
			$this->db->where("( FIND_IN_SET('".$find_in_set."', `paper_setter`) OR moderator_id like '%$find_in_set%')");
		}
		if(!empty($condition))
		{ 
			foreach($condition as $key=>$val) {
	...
	*/
	public function getCommonPMTitleArray($tbl_name='tbl_course_papersetter_moderator',$col = '*', $cond = '', $oth='title')
	{
		...
		$query = $this->db->select($col)->get($tbl_name);
		//echo $this->db->last_query(); die;
		$resp = $query->result_array();
		$results = array();
		foreach( $resp as $val){
	...
	*/
	public function getQuestionStats($cond = '')
	{
		...
		$this->db->group_by('moderator_status');
		$query = $this->db->select("moderator_status, count(*)as total")->get('tbl_questionbank');
		//echo $this->db->last_query(); die;
		$results = array();
	...
	*/
	...
	/**
	 * GET ALL CO-MAPPING ID WHILE UPLOADING COURSE CURRICULA IN BULK
	*/
	public function getCosIds($titles) {
        $this->db->select('id');
        $this->db->from('tbl_cos');
