...
Class Studentcmapping_model extends CI_Model{
	...
	*/
	...
	public function getAllstudentsmapping($academic_year_id='')
	{
		$resutls = array();
		$dbreport = $this->load->database('dbreport', TRUE);
        $sql = "SELECT 
                ...
                em.full_name,
                acm.academic_year,
                COUNT(sr.system_id) AS total_system_ids
            FROM 
                PS_S_PRD_CLS_PI_VW ps
            LEFT JOIN 
                tbl_course c ON c.id = ps.course_id
            LEFT JOIN 
                tbl_school_master smc ON smc.id = ps.school_id
            LEFT JOIN 
                tbl_department_master dmc ON dmc.id = ps.department_id
            LEFT JOIN 
                tbl_employee_master em ON em.employee_id = ps.emp_id
            LEFT JOIN 
                tbl_academicyear acm ON acm.id = ps.academic_year_id
            LEFT JOIN 
                PS_S_PRD_STDREG_VW sr ON sr.class_nbr = ps.class_nbr
            WHERE 
                ps.is_deleted = '0' 
                AND ps.academic_year_id = '".$academic_year_id."'
                AND c.status = '1'
            GROUP BY 
                ps.class_nbr
            ORDER BY 
