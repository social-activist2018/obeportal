...
Class Cfeedback_Model extends CI_Model{
...
/*
	...
	* Get All topic lists
	*/
	public function getEmployeeArray($conditions='') 
	{
		$resutls = array();
		...
		$query = $this->db2->join('tbl_employee_master', 'tbl_employee_master.employee_id = tbl_assign_room_slot_section.course_pi');
		$query = $this->db2->join('tbl_school_master', 'tbl_school_master.id = tbl_employee_master.school_id');
		$query = $this->db2->join('tbl_department_master', 'tbl_department_master.id = tbl_employee_master.department_id');
		$this->db2->distinct();
		$query = $this->db2->select("tbl_employee_master.employee_id,full_name, tbl_employee_master.school_id, tbl_employee_master.department_id, school_name, name")->get('tbl_assign_room_slot_section');
		#echo $this->db2->last_query();die;
		return $query->row_array();
	} 

public function getallFeedbackList($course_id, $employee_id, $academic_year_id, $turn, $tbl_name = 'tbl_course_feedback_2401_1',$ref_tbl_name ='tbl_course_feedback_ref_2401_1' )
	{
		$resutls = array();
		$dbreport = $this->load->database('dbreport', TRUE);
	...
	$sql = "SELECT 
				...
				rsp.question, 
				qus.course_id, 
				COUNT(DISTINCT CASE WHEN answer = '1' THEN qus.system_id END) AS answer1_count, 
				COUNT(DISTINCT CASE WHEN answer = '2' THEN qus.system_id END) AS answer2_count, 
				COUNT(DISTINCT CASE WHEN answer = '3' THEN qus.system_id END) AS answer3_count, 
				COUNT(DISTINCT CASE WHEN answer = '4' THEN qus.system_id END) AS answer4_count, 
				COUNT(DISTINCT CASE WHEN answer = '5' THEN qus.system_id END) AS answer5_count, 
				CASE 
					WHEN ref.ques_id = 0 THEN GROUP_CONCAT(DISTINCT answer ORDER BY answer SEPARATOR ', ') 
					ELSE NULL 
				END AS concatenated_answers,
				COUNT(DISTINCT qus.system_id) AS total_responses,  -- Count unique students per question
				COUNT(DISTINCT qus.system_id) AS total_students  -- This should now match total_responses
			FROM ".$tbl_name." qus 
			JOIN ".$ref_tbl_name." ref ON qus.reference = ref.reference 
			LEFT JOIN tbl_coursefeedbackquestion sfq ON sfq.id = ref.ques_id 
			LEFT JOIN tbl_questionfeedbackrepository rsp ON rsp.id = ref.ques_id 
			WHERE 
			qus.faculty_id = '".$employee_id."' 
							AND qus.course_id = '".$course_id."' 
							AND qus.turn = '".$turn."' 
							AND rsp.question!=''
							AND qus.academic_year_id='".$academic_year_id."' 
			  GROUP BY qus.course_id, ref.ques_id;";
...
public function getallFeedbackCommentList($course_id, $employee_id, $academic_year_id, $turn,$tbl_name = 'tbl_course_feedback_2401_1',$ref_tbl_name ='tbl_course_feedback_ref_2401_1' )
	{
		$resutls = array();
		$dbreport = $this->load->database('dbreport', TRUE);
		$sql = "SELECT 
				qus.system_id, answer
			FROM 
				$tbl_name qus
			JOIN 
				$ref_tbl_name ref ON qus.reference = ref.reference
