...
Class Survey_Model extends CI_Model{
		...
		extract($_POST);
			foreach($qid as $k => $v){
				...
				//if($quid=='5') {
					...
					//echo "INSERT INTO survey_answers set $data"; die;
				//}
				//echo "INSERT INTO survey_answers set $data"; die;
				$save[] = $this->db->query("INSERT INTO survey_answers SET $data");
			}
		if(isset($save))
		return 1;
	}
	...
	*/	
	...
	public function customQuerySIngleRecords($id, $system_id){
		$response = array();
		if($id>0){
			$sql = "SELECT a.*,q.type from survey_answers a inner join survey_questions q on q.id = a.question_id where a.survey_id ={$id} AND a.system_id ={$system_id}";
	...
	public function customQueryRecords($id){
		$response = array();
		if($id>0){
			$sql = "SELECT a.*,q.type from survey_answers a inner join survey_questions q on q.id = a.question_id where a.survey_id ={$id}";
			$results = $this->db->query($sql);
			$response = $results->result_array();
		}
		return $response;
	...
	*/
    ...
    {
        $result = $this->db->query("SELECT distinct(system_id) FROM survey_answers where is_deleted='0' AND survey_id = '".$survey_id."'");
        return $result->num_rows();
