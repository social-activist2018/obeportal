...
Class Vissionmission_Model extends CI_Model{
	...
	public function __construct()
	{
		...
		public function get_distinct_records_list(
			$tbl_name = 'tbl_missionstatement',
			$col = '*',
			...
			$this->db2->distinct();
			$this->db2->select(!empty($col) ? $col : '*');
			$this->db2->from($tbl_name);

			if (!empty($conditionArray)) {
				$this->db2->where($conditionArray);
			}

			if (!empty($orderBy)) {
				$this->db2->order_by($orderBy, $orderDir);
			}
			if (!empty($group_by)) {
	...
	public function get_active_mappings($conditionArray ='')
    {
        $this->db2->select('
            crr.id, 
            crr.display_order, 
            ...
            crr.createdon, 
            crr.modifiedon, 
            vs.vission_statement, 
            vs.points AS vision_points, 
            ms.mission, 
            ms.points AS mission_points
        ');
        $this->db2->from('tbl_correlationmapping AS crr');
        $this->db2->join('tbl_vissionstatement AS vs', 'crr.vission_id = vs.id', 'inner');
        $this->db2->join('tbl_missionstatement AS ms', 'crr.mission_id = ms.id', 'inner');
        $this->db2->where('crr.status', '1');
        $this->db2->where('vs.status', '1');
        $this->db2->where('ms.status', '1');
		if(!empty($conditionArray)){
		...
		foreach($response as $rec){
			$respArray[$rec['vision_points']][] = $rec;
		}
