...
Class Events_Model extends CI_Model{
	...
	*/
	...
	/*
	* Function : getAllBookingOrderRecords
	*/
	public function getAllBookingOrderRecords($condition=null, $betweenDate = '')
	{
        if(!empty($createdon)){
		  $this->db2->where("DATE_FORMAT(tbl_order_master.createdon, '%Y-%m-%d') <= '$createdon'");
		}
		if(!empty($condition))
		{ 
			foreach($condition as $key=>$val) {
				if($key=='from_date' || $key=='to_date') {
					$this->db2->where("DATE_FORMAT(tbl_order_master.createdon,'%m/%d/%Y') >='$val'");
				}  else {
					$this->db2->where($key, $val);
				}
			}
		...
		if(!empty($betweenDate))
		{
			$from_date = $betweenDate['from_date'];
			$to_date = $betweenDate['to_date'];
			$this->db2->where("DATE_FORMAT(tbl_order_master.createdon,'%m/%d/%Y') >='$from_date'");
			$this->db2->where("DATE_FORMAT(tbl_order_master.createdon,'%m/%d/%Y') <='$to_date'");
		}
		$this->db2->order_by('tbl_order_master.createdon', 'asc');
		$query = $this->db2->join('tbl_booking_confirmation', 'tbl_booking_confirmation.id = tbl_order_master.booking_confirmation_id');
		$query = $this->db2->join('tbl_user_registration', 'tbl_user_registration.id = tbl_order_master.reg_id');
		$query = $this->db2->select("tbl_order_master.id as orid,tbl_order_master.order_id, tbl_order_master.payment_method, tbl_order_master.status as ostatus,tbl_user_registration.*,tbl_booking_confirmation.slot_date, tbl_booking_confirmation.address,tbl_booking_confirmation.pincode,tbl_booking_confirmation.slot_id,tbl_booking_confirmation.test_id  ")->get('tbl_order_master');
		//echo $this->db2->last_query();die; 
		return $query->result_array();
    }
	/*
	...
	*/
	...
	{
        $this->db2->where('tbl_event_master.is_deleted', '0');
        if(!empty($createdon)){
		...
		{ 
			foreach($condition as $key=>$val) {
				if($key=='from_date' || $key=='to_date') {
					$this->db2->where("DATE_FORMAT(tbl_event_master.createdon,'%m/%d/%Y') >='$val'");
				}  else {
					$this->db2->where($key, $val);
				}
			}
		...
		if(!empty($betweenDate))
		{
			$from_date = $betweenDate['from_date'];
			$to_date = $betweenDate['to_date'];
			$this->db2->where("DATE_FORMAT(tbl_event_master.createdon,'%m/%d/%Y') >='$from_date'");
			$this->db2->where("DATE_FORMAT(tbl_event_master.createdon,'%m/%d/%Y') <='$to_date'");
		}
		$this->db2->order_by('tbl_event_master.createdon', 'asc');
		$query = $this->db2->join('tbl_campus', 'tbl_campus.id = tbl_event_master.campus_id');
		$query = $this->db2->join('tbl_admin', 'tbl_admin.id = tbl_event_master.author_id');
		$query = $this->db2->select("tbl_event_master.*, , tbl_campus.campus_name, tbl_admin.first_name,tbl_admin.last_name")->get('tbl_event_master');
		//echo $this->db2->last_query();die;
		return $query->result_array();
    }
	...
	*/
	...
	public function getAlldata($event_id){
	   $this->db2->select("tbl_event_organizer_contactinfo.contact_name,tbl_event_organizer_contactinfo.phone,tbl_event_organizer_contactinfo.email,tbl_event_organizer_contactinfo.website_url,tbl_event_master.*");
       //$this->db2->from('tbl_event_master');
	   $this->db2->where('tbl_event_master.id',$event_id);
       $this->db2->join('tbl_event_organizer_contactinfo', 'tbl_event_organizer_contactinfo.event_id = tbl_event_master.id');
       $query = $this->db2->get('tbl_event_master');
	   //echo $this->db2->last_query(); die;
	   	$results = array();
		$row = $query->row_array();
foreach ($query->result() as $row)
{
       $results['contact_name']=$row->contact_name;
       $results['phone']=$row->phone;
       $results['email']=$row->email;
       $results['website_url']=$row->website_url;
}
		//print_r($results);die;
		return $results;
}
 	public function getAllcost($event_id){
	   $this->db2->select("tbl_event_cost_tickets.cost,tbl_event_cost_tickets.free_event,tbl_event_cost_tickets.ticket_type,tbl_event_master.*");
       //$this->db2->from('tbl_event_master');
	   $this->db2->where('tbl_event_master.id',$event_id);
       $this->db2->join('tbl_event_cost_tickets', 'tbl_event_cost_tickets.event_id = tbl_event_master.id');
       $query = $this->db2->get('tbl_event_master');
	   //echo $this->db2->last_query(); die;
	   		$results = array();
		$row = $query->row_array();
foreach ($query->result() as $row)
{
       ...
       $results['ticket_type']=$row->ticket_type;
       $results['free_event']=$row->free_event;
}
		//print_r($results);die;
		return $results;
}
 	public function getAlllocation($event_id){
	   $this->db2->select("tbl_event_location_details.address,tbl_event_location_details.venue_name,tbl_event_location_details.latitude,tbl_event_location_details.longitude,tbl_event_location_details.input_coordinates,tbl_event_master.*");
       //$this->db2->from('tbl_event_master');
	   $this->db2->where('tbl_event_master.id',$event_id);
       $this->db2->join('tbl_event_location_details', 'tbl_event_location_details.event_id = tbl_event_master.id');
       $query = $this->db2->get('tbl_event_master');
	   //echo $this->db2->last_query(); die;
	   		$results = array();
		$row = $query->row_array();
foreach ($query->result() as $row)
{
       ...
       $results['venue_name']=$row->venue_name;
       $results['latitude']=$row->latitude;
       $results['longitude']=$row->longitude;
       $results['input_coordinates']=$row->input_coordinates;
}
		//print_r($results);die;
		return $results;
}
 	public function getAlldate($event_id){
	   $this->db2->select("tbl_event_date_time.start_date,tbl_event_date_time.start_time,tbl_event_date_time.end_date,tbl_event_date_time.end_time,tbl_event_date_time.time_zone,tbl_event_date_time.all_day_event,tbl_event_date_time.no_end_time,tbl_event_master.*");
       //$this->db2->from('tbl_event_master');
	   $this->db2->where('tbl_event_master.id',$event_id);
       $this->db2->join('tbl_event_date_time', 'tbl_event_date_time.event_id = tbl_event_master.id');
       $query = $this->db2->get('tbl_event_master');
	   //echo $this->db2->last_query(); die;
	   		$results = array();
		$row = $query->row_array();
foreach ($query->result() as $row)
{
       ...
       $results['time_zone']=$row->time_zone;
       $results['all_day_event']=$row->all_day_event;
       $results['no_end_time']=$row->no_end_time;
}
		//print_r($results);die;
		return $results;
}
...
/*
	...
	*/
	...
	{
			$this->db2->select('tbl_event_master.*, tbl_schools.school_name,tbl_event_date_time.start_date,tbl_event_date_time.start_time, tbl_event_date_time.end_date,tbl_event_date_time.end_time,tbl_event_date_time.time_zone,tbl_event_location_details.venue_name,tbl_event_organizer_contactinfo.contact_name,tbl_event_organizer_contactinfo.phone,tbl_event_organizer_contactinfo.email,tbl_event_organizer_contactinfo.website_url');
			$this->db2->join('tbl_event_date_time', 'tbl_event_date_time.event_id = tbl_event_master.id', 'left');
			$this->db2->join('tbl_schools', 'tbl_schools.id = tbl_event_master.organizing_school', 'left');
			$this->db2->join('tbl_event_organizer_contactinfo', 'tbl_event_organizer_contactinfo.event_id = tbl_event_master.id', 'left');
			$this->db2->join('tbl_event_location_details', 'tbl_event_location_details.event_id = tbl_event_master.id', 'left');
			if($id>0){
			 $this->db2->where('tbl_event_master.id', $id);	
			}
			if(!empty($condition)){
				foreach($condition as $key=>$val){
	...
	public function getAllComonInList($tbl_name, $col = ' * ', $condition=null, $order_by = NULL, $where_in='')
    {
        ...
        $this->db2->where('is_deleted', '0');
		if($where_in!=''){
        $this->db2->where_in('status', $where_in);
		}
		if(!empty($condition))
		{ 
	...
	* Function : getAllRecords
	*/
	public function getAllRecordss($tbl_name, $col = ' * ', $condition=null, $order_by = NULL,$limit=NULL, $start=NULL)
    {
        $time = time();
        $this->db2->select($col);
        $this->db2->where('is_deleted', '0');
