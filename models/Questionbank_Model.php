...
Class Questionbank_Model extends CI_Model{
	...
	*/	
	public function courseWiseTQuesCount($course_id = '', $academic_id='') 
	{
		$where = '';
		if($course_id) {
			$where .= ' AND course_id = "'.$course_id.'"';
		...
		if($academic_id>0) {
			$where .= ' AND qb_academic_year_id = "'.$academic_id.'"';
		}
	...
	*/	
	public function courseWiseQuesCount($course_id = '', $academic_id='') 
	{
		$where = '';
		if($course_id) {
			$where .= ' AND course_id = "'.$course_id.'"';
	...
	*/
	...
	/*
	* Function : getAllModeRecords
	*/
	public function getAllModeRecords($tbl_name, $col = ' * ', $condition=null, $where_in = null, $order_by = NULL,$limit=NULL, $start=NULL, $likeSearch=NULL)
    {
        $time = time();
        $this->db2->select($col);
        $this->db2->where('is_deleted', '0');
		...
		if($where_in){
         $this->db2->where_in('unit_id', $where_in);
		}
	...
	*/
	...
	/*
	* Function : getCommonAltPMArray
	*/
	public function getCommonAltPMArray($tbl_name='tbl_course_papersetter_moderator', $col='*', $cond = '')
	{
		if(!empty($cond)) {
			foreach($cond as $key=>$val) {
				$this->db2->where($key, $val);
	...
	*/
	public function getCommonPMArray($tbl_name='tbl_course_papersetter_moderator', $cond = '' , $col='*')
	{
		if(!empty($cond)) {
			foreach($cond as $key=>$val) {
				$this->db2->where($key, $val);
	...
	*/
	...
	public function insert($tbl_name='', $post)
    {
	...
	public function update($tbl_name='', $post, $condArray)
    {
		if(!empty($condArray)){
	...
	*/
	public function getBloomsTaxonomy($btl_id, $qTitle)
	{
		$this->db2->select('id,btl_id,title');
		$this->db2->where('is_deleted', '0');
		$this->db2->where('status', '1');
		$qTitle = str_replace(array("'s",'?','.','`','@','http',':','#','','"','\'', '"',',' , ';', '<', '>', '.', '*','<p>','</p>','img','height','width'),' ', $qTitle);
		$qTitleArray = explode(' ', $qTitle);
		if(!empty($qTitleArray)){
			$orArray = array();
			foreach($qTitleArray as $rvalue){
				$frvalue = '';
				$frvalue = str_replace(array("'s",'?','.','`','@','http',':','#','','"','\'', '"',',' , ';', '<', '>', '.', '*','<p>','</p>','img','height','width'),'', $rvalue);
	...
	public function updateinfoBYMultiCond($tbl_name='', $post, $cond)
    {
		if(!empty($cond)){
	...
	public function getTeachingschemeWithCourse($condition, $where_in = NULL, $where_key='cpm_id', $limit =null, $start =null)
	{
			$results = array();
			...
			if ($limit !== null && $start !== null) {
				$query = $this->db2->get('tbl_teaching_scheme',$limit, $start);
			} else {
